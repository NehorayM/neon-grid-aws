...
--
--  * The answer key is rebuilt from the current bank (1201 questions). Two answers had been
--    corrected in the app since the first key was written, so a duel marked them wrong.
--  * A duel can be played on one practice exam (1-19) or on any. The exams are the
--    bank cut into 65s in bank order, so exam n is questions (n-1)*65 .. n*65-1.
...
language sql immutable as $$ select 65; $$;
...
-- ---------------------------------------------------------------- verify
-- expect: 1201 questions in the key, 19 exams, the new columns, one duel_find
