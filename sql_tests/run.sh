...
for f in sql_tests/harness.sql supabase_casino.sql supabase_casino_v2.sql supabase_duel.sql supabase_exam_history.sql sql_tests/suite.sql sql_tests/exam_history_suite.sql; do
  docker cp "$f" $NAME:/tmp/ >/dev/null
done

for f in harness.sql supabase_casino.sql supabase_casino_v2.sql supabase_duel.sql supabase_exam_history.sql; do
  echo "--- installing $f"
